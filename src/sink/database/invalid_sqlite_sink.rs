...
#[derive(DataSink)]
pub struct InvalidSqliteSink {
    inner: SqliteDatabaseSink,
    flow_state: String,
}

impl InvalidSqliteSink {
    pub fn new(config_path: &str) -> Self {
        let text = std::fs::read_to_string(config_path)
            .unwrap_or_else(|_| panic!("InvalidSqliteSink: cannot read config: {}", config_path));
        let cfg: InvalidSqliteConfig = serde_json::from_str(&text)
            .unwrap_or_else(|_| panic!("InvalidSqliteSink: invalid config: {}", config_path));
        let inner = SqliteDatabaseSink::new(&cfg.database_config);
        inner.configure(&cfg.database_config, &cfg.table_name)
            .expect("InvalidSqliteSink: failed to configure database");
        info!("invalid_sqlite: configured, table={}", cfg.table_name);
        Self { inner, flow_state: cfg.flow_state }
    }
}

#[async_trait]
impl DataSink for InvalidSqliteSink {
